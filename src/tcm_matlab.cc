...
double run_tcm_distcon_sem (double *r_mat, double *param_mat, unsigned int R, 
			    unsigned int P, unsigned int N,
			    double *itemno_mat, double *sem_vec,
			    double *sem_mat, unsigned int n_list,
  ...
  vector< vector<unsigned int> > pres_itemno (n_list, vector<unsigned int> (N));
  vector< vector<double> > semvec (sem_rows, vector<double> (sem_cols));
  vector< vector<double> > sem (sem_cols, vector<double> (sem_cols));
  double logl = 0;
  bool isdc = true;
...
void mexFunction (int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
  ...
  if (nrhs == 3) {
    ...
    if (sem_type == 1) {
      ...
      double * sem_mat2 = mxGetPr(prhs[6]);
      unsigned int sem_rows2 = mxGetM(prhs[6]);
      unsigned int sem_cols2 = mxGetN(prhs[6]);
      logl = run_tcm_distcon_sem(r_mat, param_mat, R, P, N, itemno_mat,
				 sem_mat, sem_mat2, n_list,
				 sem_rows, sem_cols);
    }
  }
